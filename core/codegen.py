...
class Memory:
    ...
    def assign_memory(self, id: str, table: dict) -> int:
        if self.memory == 0:
...
class CodeGenerator:
    ...
    def generate_statement(self, stm: Statement) -> str:
        if isinstance(stm, Return):
            ...
            if stm.else_statement:
                ...
                return (
                    ...
                    "    cmpq    $0, %rax\n"
                    f"    je    _{el}\n" +
                    self.generate_block(stm.if_statement) +
                    f"    jmp    _{end}\n"
                    f"_{el}:\n" +
                    self.generate_block(stm.else_statement) +
                    f"_{end}:\n"
                )
            else:
                return (
        ...
        # Declare kept here as for sake of compactness, although technically not a statement
            ...
            variable = stm.id
            offset = self.memory.assign_memory(variable.id, symbol.table)
            return (
    ...
    def generate_exp(self, exp: Exp) -> str:
        if isinstance(exp, CommaExp):
            ...
            offset = self.search_blocks(id=exp.id.id)
            return (
            ...
            offset = self.search_blocks(id=exp.id)
            if exp.prefix == True:
                return (
